...
def restore_disk_tier_from_drive(sample_dirs, drive_backup_root):
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=32) as executor:
        ...
        # Wrap the results iterator in tqdm for progress tracking
        for future in tqdm(concurrent.futures.as_completed(futures), total=len(sample_dirs), desc="Restoring Disk Cache"):
            try:
                ...
                # Silently catch and continue if a single file copy fails; 
                # __getitem__ will self-heal it later during training
                pass
...
def build_or_load_ram_cache(all_sample_dirs, vggt, device, backup_dir, chunk_size=500):
    ...
    Uses chunked loading and saving (e.g., 500 samples/chunk) to prevent
    OOM and Drive-FUSE write failures when saving ~43GB of tensors.
    ...
    # 1. Load existing chunks from Drive
    ...
    if missing_dirs:
        ...
        for i in range(0, len(missing_dirs), chunk_size):
            ...
            # Save the ~3GB chunk to Drive BEFORE merging to minimize peak memory pressure
