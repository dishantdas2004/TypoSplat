...
def main():
    ...
    parser.add_argument("--checkpoint_dir", type=str, default="/content/drive/MyDrive/TypoSplat/stage1_5_color_finetune", help="Output dir")
    ...
    # --- Discover and Filter Directories ---
    ...
    # --- Construct Multiprocessing Datasets ---
    ...
    # --- Initialize Models & Load Checkpoint ---
    ...
    # --- Setup Optimizer (Base Params Only) ---
    ...
    # --- Baseline PSNR / SSIM Check (Epoch 0) ---
    ...
    for epoch in range(1, num_epochs + 1):
        ...
        for batch_idx, batch in enumerate(tqdm(train_dataloader, desc=f"Epoch {epoch}/{num_epochs}")):
            ...
            for sample_idx, data_cpu in enumerate(batch):
                ...
                with torch.autocast('cuda', dtype=torch.float16):
                    ...
                    # Calibrator is frozen, but its outputs still scale/shift the depth computation
            ...
            # Optimizer Step (AMP safely unscales gradients)
        ...
        # --- Periodic Visualizations (Visual Color Check) ---
        if epoch % 1 == 0:
            upsampler.eval()
            decoder.eval()
            with torch.no_grad():
